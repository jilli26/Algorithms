...
//FASTER TO USE SORT AND COMPARE THESE TWO
function anagrams(stringA, stringB) {
  const modifiedStringA = stringA.replace(/[^\w]/g, "").toLowerCase().split('').sort().join('')
  const modifiedStringB = stringB.replace(/[^\w]/g, "").toLowerCase().split('').sort().join('')

  // if (modifiedStringA !== modifiedStringB) {
  //   return false
  // } else {
  //   return true
  // }
